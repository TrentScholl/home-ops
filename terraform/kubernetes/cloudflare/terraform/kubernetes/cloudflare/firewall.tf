...
resource "cloudflare_filter" "plex_notifications" {
  ...
  expression  = "(http.host eq \"plex.trentscholl.com\" and http.request.uri.path contains \"/:/eventsource/notifications\")"
}
...
resource "cloudflare_filter" "domain_github_flux_webhook" {
  ...
  expression  = "(http.host eq \"flux-webhook.trentscholl.com\" and ip.geoip.asnum eq 36459)"
}
